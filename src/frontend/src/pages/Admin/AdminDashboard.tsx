...
export default function AdminDashboard() {
  ...
  const [eventThumbnailFile, setEventThumbnailFile] = useState<File | null>(
    null,
  );
  ...
  const [roadClosures, setRoadClosures] = useState<RoadClosure[]>([
    {
      id: 1,
      road_name: "Đường Bạch Đằng",
      event_title: "Lễ hội Pháo hoa quốc tế DIFF 2026",
      restriction_type: "CLOSED",
      time_frame: "18:00 - 23:00",
    },
    {
      id: 2,
    ...
    {
      id: 3,
      road_name: "Đường 2 Tháng 9",
      event_title: "Danang International Marathon",
      restriction_type: "ONE_WAY",
      time_frame: "04:00 - 10:00",
    },
  ]);
  ...
  const fetchFloodZones = async () => {
    try {
      ...
      showPremiumToast(
        "Không thể lấy dữ liệu vùng ngập lụt từ server.",
        "error",
      );
    }
  };
    ...
    try {
      ...
      if (response.data && response.data.success) {
        const data = response.data.data.map((alert: any) => ({
          ...
          created_at: new Date(alert.created_at).toLocaleString("vi-VN", {
            hour: "2-digit",
            minute: "2-digit",
            day: "2-digit",
            month: "2-digit",
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      if (result && result.data) {
        if (result.data.has_password !== undefined)
    ...
    try {
      ...
      const mappedClosures = data.map((road) => {
        ...
        if (road.days_of_week) {
          const daysStr = road.days_of_week
            .split(",")
            .map((d: string) => {
            ...
            .join(", ");
          const startStr = road.start_time_of_day
            ? road.start_time_of_day.substring(0, 5)
            : "";
          const endStr = road.end_time_of_day
            ? road.end_time_of_day.substring(0, 5)
            : "";
          timeFrameStr = `${startStr} - ${endStr} (${daysStr} hàng tuần)`;
        } else {
          const startStr = new Date(road.restriction_start).toLocaleTimeString(
            "vi-VN",
            { hour: "2-digit", minute: "2-digit" },
          );
          const endStr = new Date(road.restriction_end).toLocaleTimeString(
            "vi-VN",
            { hour: "2-digit", minute: "2-digit" },
          );
          const dateStr = new Date(road.restriction_start).toLocaleDateString(
            "vi-VN",
          );
  ...
  const fetchEvents = async () => {
    ...
    try {
      ...
      if (!token) {
        throw new Error("Không tìm thấy token đăng nhập Admin.");
      }
      ...
      const responses = await Promise.all(
        statuses.map(async (status) => {
          const response = await fetch(
            `${BASE_URL}/api/events?status=${status}`,
            {
              method: "GET",
              headers: {
          ...
          if (!response.ok) {
            throw new Error(
              result.message || `Không thể tải sự kiện có trạng thái ${status}`,
            );
          }
      ...
      const allEvents: DBEvent[] = responses.flat();

      // Chống trùng theo event_id nếu API trả lặp.
      const uniqueEvents = Array.from(
        new Map(
          allEvents.map((event: DBEvent) => [event.event_id, event]),
        ).values(),
      );

      // Sự kiện mới cập nhật hiển thị trước.
      ...
      console.error("Lỗi tải danh sách sự kiện Admin:", error);

      setEvents([]);

      showPremiumToast(
        error.message || "Không thể tải danh sách sự kiện từ server.",
        "error",
      );
    } finally {
  ...
  const handleCreateEvent = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formData.append(
        "short_description",
        eventFormData.short_description || "",
      );
      ...
      formData.append(
        "is_featured",
        String(editingEvent ? editingEvent.is_featured : false),
      );
      formData.append(
        "is_free",
        String(editingEvent ? editingEvent.is_free : true),
      );
      formData.append(
        "ticket_price",
        String(editingEvent ? editingEvent.ticket_price : 0),
      );
      if (eventBannerFile) {
  ...
  const handleCreateTrafficAlert = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        showPremiumToast(
          response.data.message || "Lỗi khi tạo cảnh báo giao thông.",
          "error",
        );
      }
    } catch (error) {
  ...
  const toggleFloodStatus = async (id: number) => {
    ...
    try {
      ...
      if (response.data && response.data.success) {
        showPremiumToast(
          "Cập nhật trạng thái vùng ngập thành công!",
          "success",
        );
        setFloodZones((prev) =>
          prev.map((z) =>
            z.id === id
              ? { ...z, is_active: nextStatus, last_updated: "Vừa xong" }
              : z,
          ),
        );
      } else {
  ...
  const handleSetup2FA = async () => {
    try {
      ...
      setTwoFaQRCode(
        "https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=otpauth://totp/DanangSmart:admin",
      );
  ...
  const handleDisable2FA = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setTwoFaError(true);
        setTwoFaMessage(
          result.error?.message || result.message || "Lỗi khi tắt 2FA",
        );
      }
    } catch (error: any) {
      setTwoFaError(true);
      const errorMessage =
        ...
        "Mật khẩu không chính xác hoặc lỗi hệ thống!";
  ...
  return (
    ...
    >
      ...
      {activeMenu === "traffic" && (
        <TrafficTab
          trafficAlerts={trafficAlerts}
          toggleTrafficStatus={toggleTrafficStatus}
          showTrafficModal={showTrafficModal}
          setShowTrafficModal={setShowTrafficModal}
          trafficFormData={trafficFormData}
          setTrafficFormData={setTrafficFormData}
      ...
      {activeMenu === "flood" && (
        <FloodTab
          floodZones={floodZones}
          toggleFloodStatus={toggleFloodStatus}
        />
      )}
      {activeMenu === "closure" && (
        <ClosureTab
      ...
      {activeMenu === "settings" && (
        <SettingsTab
          ...
          showTwoFaQR={showTwoFaQR}
          setShowTwoFaQR={setShowTwoFaQR}
          showDisable2FaInput={showDisable2FaInput}
          setShowDisable2FaInput={setShowDisable2FaInput}
