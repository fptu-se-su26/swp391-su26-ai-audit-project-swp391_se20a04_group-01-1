  ...
  React.useEffect(() => {
    if (showTrafficModal && mapRef.current) {
      mapRef.current.setCenter([
        trafficFormData.longitude || 108.2022,
        trafficFormData.latitude || 16.0544,
      ]);
    }
      ...
      <div className="space-y-6 animate-fade-in">
        <div className="bg-white p-4 rounded-2xl border border-slate-200 flex items-center justify-between shadow-sm">
          <h3 className="text-base font-bold text-slate-800">
            Thông báo Sự cố giao thông thời gian thực
          </h3>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {trafficAlerts.map((alert) => (
            <div
              key={alert.id}
              className={`bg-white rounded-2xl border p-5 shadow-sm relative overflow-hidden transition ${alert.is_active ? "border-orange-200" : "border-slate-200 opacity-60"}`}
            >
              <div
              ...
              <h4 className="font-bold text-slate-800 text-sm mt-3 leading-snug">
                {alert.title}
              </h4>
      ...
      {showTrafficModal && (
        <div className="fixed inset-0 bg-slate-900/60 flex items-center justify-center z-50 p-4 animate-fade-in">
          <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] shadow-2xl flex flex-col border border-slate-200 animate-slide-up overflow-hidden">
            ...
            <form
              onSubmit={handleCreateTrafficAlert}
            ...
            >
              <div className="flex-1 grid grid-cols-1 md:grid-cols-12 min-h-0 overflow-hidden">
                ...
                <div className="col-span-1 md:col-span-6 overflow-y-auto p-6 space-y-4 custom-scrollbar border-r border-slate-100 text-slate-700 text-left">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase mb-1">
                      Vùng ảnh hưởng (Polygon - GeoJSON)
                    </label>
                    <textarea
                      ...
                      onChange={(e) =>
                        setTrafficFormData({
                          ...trafficFormData,
                          affected_area_polygon: e.target.value,
                        })
                      }
                      rows={4}
                    ...
                    <p className="text-[10px] text-slate-400 mt-1">
                      Tuỳ chọn: Nhập chuỗi GeoJSON chuẩn để hiển thị đa giác
                      vùng ảnh hưởng lên bản đồ.
                    </p>
                  </div>
                </div>
