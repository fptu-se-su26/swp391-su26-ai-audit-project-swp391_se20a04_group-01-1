...
import { useNavigationEngine } from "../Home/hooks/useNavigationEngine";
...
export default function TrackRoutePage() {
  ...
  const [sharerCoords, setSharerCoords] = useState<{
    lat: number;
    lng: number;
    heading: number;
  } | null>(null);
    ...
    const fetchSessionStatus = async () => {
      try {
        ...
        if (data.success && data.session) {
          ...
          setErrorMsg(
            data.message || "Không thể tìm thấy thông tin chia sẻ vị trí.",
          );
        }
      } catch (err) {
  ...
  const calculateRouteToSharer = async (force = false) => {
    if (!viewerLocation || !sharerCoords) {
      showPremiumToast(
        "Đang xác định tọa độ... Vui lòng bật định vị GPS.",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      showPremiumToast(
        "Lỗi tính toán tuyến đường dẫn đến người chia sẻ.",
        "error",
      );
    } finally {
  ...
  return (
    <div className="w-screen h-screen relative overflow-hidden font-sans">
      ...
      <div className="absolute top-4 left-4 z-10 max-w-xs bg-white/95 backdrop-blur-md p-4 rounded-3xl shadow-xl border border-slate-100 flex flex-col gap-3">
        ...
        <div className="border-t border-slate-100 pt-2 flex flex-col gap-2">
          <p className="text-[10px] text-slate-500 leading-normal">
            Liên kết này cho phép xem vị trí chuyển động trực tuyến của người
            chia sẻ cứu nạn.
          </p>
          ...
          <div className="flex gap-2 mt-1">
            ...
            {viewerLocation && (
              ...
              >
                <Navigation
                  size={12}
                  className={isRoutingActive ? "rotate-45" : ""}
                />
      ...
      {routeData && (
        <div className="absolute top-4 right-4 z-10 bg-white/95 backdrop-blur-md px-4 py-3 rounded-2xl shadow-lg border border-slate-100 text-right max-w-xxs">
          <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">
            Đường đi tới người cứu nạn
          </p>
          <p className="text-sm font-black text-blue-600 mt-0.5">
            {routeData.distanceKm} km
          </p>
          <p className="text-[10px] font-semibold text-slate-500 mt-0.5">
            Thời gian: {routeData.durationMin} phút
          </p>
          {loadingRoute && (
            <p className="text-[9px] text-amber-500 font-bold animate-pulse mt-1">
              🔄 Tự động cập nhật tuyến đường...
            </p>
          )}
        </div>
      )}
      ...
      <Map
        ...
        initialViewState={{
          longitude: sharerCoords?.lng || 108.20623,
      ...
      >
        ...
        {sharerCoords && (
          <Marker
            longitude={sharerCoords.lng}
            latitude={sharerCoords.lat}
            anchor="center"
          >
