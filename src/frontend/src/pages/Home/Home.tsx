...
export default function Home() {
    ...
    const [mapControls, setMapControls] = useState({
        layers: true,      // Mặc định bật theo thiết kế (Màu Tím)
        traffic: true,     // Mặc định bật theo thiết kế (Màu Đỏ)
        flood: false       // Mặc định tắt (Màu Trắng, Icon Xanh)
    });

    useEffect(() => {
        if (activeTab === 'map' && showAlertPopup) {
    ...
    // Hàm toggle bật/tắt các lớp bản đồ bên phải
    ...
    if (activeTab === 'map') {
        return (
            <div className="w-full h-screen relative bg-slate-100 overflow-hidden font-sans select-none">
                ...
                <div className="absolute top-6 left-6 right-6 z-10 flex items-center justify-between gap-4 pointer-events-none">
                    ...
                    <div className="flex items-center gap-3 flex-1 max-w-[calc(100%-120px)] overflow-hidden pointer-events-auto">
                        <div className="w-72 h-[42px] bg-white rounded-full shadow-md border border-slate-200/60 flex items-center px-4 shrink-0">
                            ...
                            <input
                                type="text"
                                placeholder="Tìm kiếm tại Đà Nẵng..."
                    ...
                    <div className="flex items-center gap-3 shrink-0 pointer-events-auto relative">
                        ...
                        <button
                            onClick={() => { setActiveTab('profile'); setShowNotificationModal(false); }}
                            className="w-[42px] h-[42px] flex items-center justify-center bg-white rounded-full shadow-md border border-slate-200/60 text-slate-600 hover:text-blue-600 transition-all"
                        >
                            <User size={18} />
                        </button>
                ...
                {/* Tái tạo chính xác theo mẫu thiết kế image_b122e8.png */}
    ...
    // ================= GIAO DIỆN MÀN HÌNH CÁ NHÂN =================
    return (
        <div className="w-full min-h-screen bg-slate-50 font-sans text-slate-800">
            <div className="w-full bg-gradient-to-r from-blue-600 to-indigo-700 text-white px-8 py-8 relative shadow-md">
                <button
                ...
                <button className="absolute top-6 right-6 p-2 bg-white/10 hover:bg-white/20 rounded-full text-white transition-all">
                    <Settings size={16} />
                </button>

                <div className="max-w-3xl mx-auto flex items-center gap-4 mt-4">
                    <div className="w-16 h-16 bg-white/20 rounded-full border-4 border-white/30 flex items-center justify-center text-2xl shadow-lg relative">
                        👤
                        <span className="absolute bottom-0 right-0 w-3 h-3 bg-green-400 rounded-full border-2 border-blue-600"></span>
                    </div>
                    <div>
                        <h1 className="text-lg font-bold">Nguyễn Minh Tuấn</h1>
                        <p className="text-[11px] text-white/80">Người dùng Premium - Đà Nẵng</p>
                        <div className="text-amber-300 text-[10px] mt-0.5">⭐⭐⭐⭐⭐ <span className="text-white/60 ml-1">Thành viên tích cực</span></div>
                    </div>
                </div>

                <div className="max-w-3xl mx-auto grid grid-cols-4 gap-3 mt-6">
                    <div className="bg-white/10 backdrop-blur-md p-3 rounded-xl text-center border border-white/10">
                        <div className="text-base font-black">148</div>
                        <div className="text-[10px] opacity-80">Chuyến đi</div>
                    </div>
                    <div className="bg-white/10 backdrop-blur-md p-3 rounded-xl text-center border border-white/10">
                        <div className="text-base font-black">3.2h</div>
                        <div className="text-[10px] opacity-80">Tiết kiệm</div>
                    </div>
                    <div className="bg-white/10 backdrop-blur-md p-3 rounded-xl text-center border border-white/10">
                        <div className="text-base font-black">1,240</div>
                        <div className="text-[10px] opacity-80">Km lộ trình</div>
                    </div>
                    <div className="bg-white/10 backdrop-blur-md p-3 rounded-xl text-center border border-white/10">
                        <div className="text-base font-black">28</div>
                        <div className="text-[10px] opacity-80">Sự kiện tham gia</div>
                    </div>
                </div>
            </div>

            <div className="max-w-3xl mx-auto p-6 flex flex-col gap-3">
                <div className="w-full bg-red-50 rounded-xl p-3 border border-red-100 flex items-center justify-between cursor-pointer hover:bg-red-100/50 transition-colors">
                    <div className="flex items-center gap-2.5">
                        <div className="p-2 bg-red-100 rounded-lg text-red-600"><ShieldAlert size={14} /></div>
                        <div>
                            <div className="text-xs font-bold text-red-900">Cảnh báo ngập lụt</div>
                            <div className="text-[10px] text-red-600">2 điểm nóng ngập nước cần tránh</div>
                        </div>
                    </div>
                    <span className="text-red-400 text-xs">❯</span>
                </div>

                <div className="w-full bg-amber-50 rounded-xl p-3 border border-amber-100 flex items-center justify-between cursor-pointer hover:bg-amber-100/50 transition-colors">
                    <div className="flex items-center gap-2.5">
                        <div className="p-2 bg-amber-100 rounded-lg text-amber-600"><Ban size={14} /></div>
                        <div>
                            <div className="text-xs font-bold text-amber-900">Tuyến đường cấm</div>
                            <div className="text-[10px] text-amber-600">1 khu vực đang hạn chế phương tiện</div>
                        </div>
                    </div>
                ...
                <button
                    ...
                    className="w-full py-2.5 bg-slate-200 hover:bg-red-500 hover:text-white rounded-xl text-slate-600 text-xs font-bold transition-all flex items-center justify-center gap-1.5 mt-2"
                >
                    <LogOut size={14} />
                    ĐĂNG XUẤT TÀI KHOẢN DNPULSE
                </button>
            </div>
        </div>
    );
