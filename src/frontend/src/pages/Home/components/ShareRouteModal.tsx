        ...
        <div className="p-6 space-y-4 font-sans">
          <div className="flex gap-2">
            <input
              ...
              type="text"
              value={shareUrl}
              className="flex-1 px-3 py-2 text-[10px] bg-slate-50 rounded-xl border outline-none font-semibold text-slate-600"
            />
            <button
              onClick={() => {
                navigator.clipboard.writeText(shareUrl);
                showPremiumToast("Đã sao chép liên kết!", "success");
              }}
              className="px-3 bg-slate-100 hover:bg-slate-200 active:scale-95 transition-all rounded-xl flex items-center justify-center text-slate-600"
              title="Sao chép"
            >
              <Copy size={14} />
            </button>
