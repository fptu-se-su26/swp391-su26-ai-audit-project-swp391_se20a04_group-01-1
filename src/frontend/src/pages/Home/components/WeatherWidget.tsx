...
export function WeatherWidget({
  ...
  const fetchForecast = async (districtName: string) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/api/weather/forecast?district=${encodeURIComponent(districtName)}`,
      );
      const data = await response.json();
      if (data.success && Array.isArray(data.data)) {
        setForecastData(data.data);
        localStorage.setItem(
          `cached_forecast_${districtName}`,
  ...
  const activeWeather = weatherList.find(
    (w) => w.district === selectedDistrict,
  );

  // Các quận có mưa lớn
  const highRiskDistricts = weatherList.filter(
    (w) => w.rain1h > 50 && w.district !== selectedDistrict,
  );
  ...
  const getWeatherIcon = (
    ...
    if (lightBg) {
      ...
      if (lowerStatus === "thunderstorm") {
        return (
          <CloudLightning
            className={`${sizeClass} text-amber-500 animate-pulse`}
          />
        );
      }
      ...
      if (lowerStatus === "clear") {
        return (
          <Sun className={`${sizeClass} text-amber-500 animate-spin-slow`} />
        );
      }
    ...
    if (lowerStatus === "rain") {
      return (
        <CloudRain className={`${sizeClass} text-blue-100 animate-pulse`} />
      );
    }
    if (lowerStatus === "thunderstorm") {
    ...
    if (lowerStatus === "clear") {
      return (
        <Sun className={`${sizeClass} text-yellow-100 animate-spin-slow`} />
      );
    }
    if (lowerStatus === "clouds") {
      if (temp >= 30) {
  ...
  const getWidgetTheme = () => {
    if (!activeWeather)
      return "from-orange-400 via-amber-500 to-rose-500 text-white";
...
max-md:bottom-24
    ...
    >
      ...
      <div
        className={`relative overflow-hidden rounded-[1.5rem] md:rounded-[2.5rem] shadow-2xl border bg-gradient-to-br transition-all duration-500 md:hover:-translate-y-1 ${getWidgetTheme()}`}
      >
        ...
        {/* 3. Main Info Section */}
        {activeWeather && (
          <div className="p-4 pb-3 md:p-6 md:pb-4 flex justify-between items-start">
            ...
            <div className="flex flex-col text-left">
              ...
              <span className="text-[40px] md:text-[54px] font-light leading-none tracking-tight text-white mt-2 select-none relative">
                {Math.round(activeWeather.temp)}
                <span className="text-[16px] md:text-[20px] font-semibold absolute top-0.5 ml-1">
                  °
                </span>
              </span>

              <span className="text-[10px] font-bold text-white/80 mt-1 uppercase tracking-wider">
                Thấp: {Math.round(activeWeather.temp - 4)}° • Cao:{" "}
                {Math.round(activeWeather.temp + 3)}°
              </span>
            </div>

            {/* Cột Phải: Đồng Hồ & Địa Điểm */}
            <div className="flex flex-col items-end text-right pr-6 mt-1">
              <span className="text-[32px] font-light text-white leading-none tracking-tight">
                {timeStr}
              </span>
              <span className="text-[10px] font-bold text-white/80 tracking-wider uppercase mt-1.5">
                {dateStr}
              </span>
                ...
                >
                  {weatherList.map((w) => (
                    <option
                      key={w.district}
                      value={w.district}
                      className="text-slate-800 bg-white rounded"
                    >
                      {w.district}
                    </option>
                  ))}
        ...
        {activeWeather && activeWeather.rain1h > 50 && (
          <div className="mx-4 mb-3 bg-red-600/30 border border-red-500/20 rounded-2xl p-2.5 flex items-start gap-2 backdrop-blur-md">
            ...
            <div className="flex flex-col text-left">
              <span className="text-[9px] font-extrabold uppercase tracking-wide text-red-100">
                Cảnh báo ngập sớm
              </span>
              <span className="text-[9px] text-white/90 leading-tight mt-0.5">
                Mưa rất lớn ({activeWeather.rain1h.toFixed(1)} mm/h). Các đường
                xung quanh có nguy cơ ngập cao.
              </span>
            </div>
          </div>
        )}
        ...
        <div className="bg-black/15 border-t border-white/5 py-3 px-4 grid grid-cols-4 items-center">
          {loadingForecast ? (
            <div className="col-span-4 text-center text-[9px] text-white/60 tracking-wider">
              Đang tải dự báo...
            </div>
            ...
            miniForecasts.map((item, idx) => (
              ...
              >
                <span className="text-[8px] font-black text-white/70 tracking-wider">
                  {item.time}
                </span>
      ...
      {highRiskDistricts.length > 0 && (
        <div className="bg-red-500/90 backdrop-blur-md text-white rounded-2xl p-2.5 px-4 shadow-lg flex items-center gap-2 border border-white/10 transition-all hover:scale-102">
          ...
          <div className="flex-1 text-[9px] text-left font-black tracking-wide leading-tight uppercase">
            Mưa ngập tại Quận{" "}
            {highRiskDistricts.map((d) => d.district).join(", ")}
          </div>
          <button
            ...
            <div className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white px-5 py-4 flex justify-between items-center">
              <div>
                ...
                <p className="text-[10px] opacity-80 mt-0.5">
                  Khu vực Quận {selectedDistrict}, Đà Nẵng
                </p>
              </div>
              <button
                onClick={() => setShowForecastModal(false)}
            ...
            <div className="p-5 overflow-y-auto flex-1 max-h-[60vh] space-y-4">
              {loadingForecast ? (
                ...
                <div className="py-12 flex flex-col items-center justify-center gap-2 text-slate-500 text-xs">
                  ...
                  <span className="font-semibold">Chưa có dữ liệu dự báo</span>

                  <span className="text-[10px] text-slate-400 text-center">
                    Hiện chưa có dữ liệu dự báo cho Quận {selectedDistrict}.
                  </span>

                  <button
                    type="button"
                    onClick={() => fetchForecast(selectedDistrict)}
                    className="mt-2 flex items-center gap-1.5 rounded-lg bg-blue-500 px-3 py-2 text-[10px] font-bold text-white transition-colors hover:bg-blue-600"
                  >
                    <RefreshCw className="w-3.5 h-3.5" />
                    Thử tải lại
                  </button>
                    ...
                    <div className="bg-red-50 border border-red-100 rounded-xl p-3 flex items-start gap-2.5">
                      ...
                      <div>
                        <h4 className="text-xs font-bold text-red-800">
                          Cảnh báo ngập lụt sớm!
                        </h4>
                        <p className="text-[10px] text-red-700 mt-0.5 leading-relaxed text-left">
                          Mô hình khí tượng dự báo quận {selectedDistrict} sẽ có
                          mưa cực to (vượt ngưỡng 50 mm/h) trong vài giờ tới.
                          Nguy cơ ngập đường cực cao, hãy chủ động chọn lộ trình
                          thay thế trước khi bắt đầu hành trình.
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-4 gap-2.5">
                    {forecastData.map((item, idx) => (
                      ...
                      >
                        ...
                        <span className="text-[9px] text-slate-500 capitalize mt-1 truncate w-full">
                          {item.description}
                        </span>

                        {item.rain > 0 ? (
                          <span
                            className={`text-[8px] font-bold px-1 py-0.5 rounded-full mt-1.5 ${
                              item.rain > 50
                    ...
                    <div className="flex items-center gap-2.5">
                      ...
                      <div className="flex flex-col text-left">
                        ...
                        <span className="text-xs font-black text-slate-800">
                          {forecastData.length > 0
                            ? Math.max(
                                ...forecastData.map((item) => item.rain),
                              ).toFixed(1)
                            : "0.0"}{" "}
                          mm/h
                        </span>
