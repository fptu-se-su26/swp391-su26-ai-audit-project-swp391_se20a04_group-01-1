...
export default function EventDetailSidebar({
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setIsUploading(true);
    try {
      await eventAPI.uploadEventImage(event.event_id, formData);
  ...
  // Resolve image URL: thêm domain backend nếu là path tương đối hoặc trả về nguyên vẹn nếu là URL tuyệt đối (như ảnh cào từ AI)
    ...
    <div className="w-80 max-md:w-full max-md:fixed max-md:bottom-0 max-md:left-0 max-md:right-0 max-md:h-[50vh] max-md:max-h-[50vh] max-md:rounded-t-3xl max-md:rounded-b-none max-md:z-[450] max-md:border-t max-md:border-slate-200/80 max-md:shadow-[0_-8px_30px_rgba(0,0,0,0.12)] max-md:animate-none bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden flex flex-col h-[calc(100vh-140px)] animate-fade-left pointer-events-auto">
              ...
              {images.map((img, idx) => {
                ...
                return (
                  <div
                    ...
                    className="shrink-0 w-24 h-16 rounded-lg overflow-hidden shadow-sm border border-slate-200 snap-center relative group"
                  >
                    <button
                      type="button"
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 text-left scrollbar-none">
        ...
        <div className="flex justify-between items-center border-b border-slate-50 pb-2.5">
          <span
            className={`text-[10px] font-extrabold px-2.5 py-1 rounded-full border ${statusClass}`}
          >
            {statusText}
          </span>
          <div className="flex items-center gap-3">
            <span className="text-[10px] font-bold text-slate-400 flex items-center gap-1">
              <Eye size={11} />
              <span>{event.view_count ?? 0} lượt xem</span>
            </span>

            <span className="text-[10px] font-bold text-slate-400 flex items-center gap-1">
              <Heart size={11} className="fill-rose-500 text-rose-500" />
              <span>{event.favorite_count ?? 0} người thích</span>
            </span>
          </div>
        </div>
      ...
      {previewImage && (
        ...
        >
          <button
            ...
            className="absolute top-5 right-5 w-10 h-10 rounded-full bg-black/50 hover:bg-black/70 text-white flex items-center justify-center transition-colors"
          >
            <X size={22} />
          </button>
          ...
          <img
            ...
            alt={previewImage.caption || "Ảnh sự kiện"}
            className="
max-w-[95vw]
max-h-[90vh]
object-contain
...
"
          ...
          {previewImage.caption && (
            <div className="mt-4 px-5 py-2 rounded-xl bg-black/40 text-white text-sm text-center max-w-xl">
              {previewImage.caption}
            </div>
          )}
        </div>
