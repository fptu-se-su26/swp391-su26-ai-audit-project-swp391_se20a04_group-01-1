...
import { getPoiRatingColor } from "../../../utils/poiRatingColors";
...
export default function POIsLayer({
  ...
  const poisGeoJSON = useMemo(() => {
    ...
    filteredPois = filteredPois.filter(
      (poi) => poi.category_name === categoryName,
    );
    ...
    return {
      ...
      features: filteredPois.map((poi) => ({
        ...
        properties: {
          ...
          // ✅ Màu điểm theo mức đánh giá (sao), không còn theo category
          color: getPoiRatingColor(poi.rating),
        },
        geometry: {
          type: "Point" as const,
