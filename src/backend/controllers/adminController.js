...
const getAdminFloodZones = async (req, res) => {
    if (req.user.role !== 'admin') {
...
// PUT /api/admin/flood-zones/:id
const updateFloodZoneStatus = async (req, res) => {
    if (req.user.role !== 'admin') {
...
// GET /api/admin/traffic-alerts
const getAdminTrafficAlerts = async (req, res) => {
    if (req.user.role !== 'admin') {
        return res.status(403).json({ success: false, message: 'Truy cập bị từ chối: Cần quyền Admin!' });
    }
    try {
...
// PUT /api/admin/traffic-alerts/:id/toggle
const toggleTrafficAlert = async (req, res) => {
    if (req.user.role !== 'admin') {
        return res.status(403).json({ success: false, message: 'Truy cập bị từ chối: Cần quyền Admin!' });
    }
    try {
...
// DELETE /api/admin/traffic-alerts/:id
