...
async function runFloodAlertJob() {
  try {
    ...
    if (toInsert.length > 0) {
      ...
      console.log(
        `[Scheduler] Flood alert job: Sent ${toInsert.length} notifications to users.`,
      );
    }
  } catch (err) {
...
async function runEventReminderJob() {
  try {
    ...
    if (toInsert.length > 0) {
      ...
      await pool.request().bulk(table);
      console.log(
        `[Scheduler] Event reminder job: Sent ${toInsert.length} reminders.`,
      );
    }
  } catch (err) {
...
async function runWeatherAlertJob() {
  try {
    ...
    const adminResult = await pool.request().query(`
        ...
        `);
    const adminId =
      adminResult.recordset.length > 0 ? adminResult.recordset[0].user_id : 1;
    ...
    for (const district of districts) {
      ...
      const checkAlertResult = await pool
        .request()
        .input("location_name", sql.NVarChar, `Quận ${district}`)
        .input("title", sql.NVarChar, "⚠️ Cảnh báo ngập lụt sớm").query(`
                    SELECT alert_id 
                    FROM TrafficAlerts 
      ...
      if (hasHeavyRain) {
        ...
        if (!existingAlert) {
          ...
          const insertAlertResult = await pool
            .request()
          ...
          console.log(
            `[Scheduler] Created weather warning for ${district} (alert_id: ${alertId})`,
          );
          ...
          if (usersResult.recordset.length > 0) {
            ...
            table.columns.add("zone_id", sql.Int, { nullable: true });
            table.columns.add("title", sql.NVarChar(200), { nullable: true });
            table.columns.add("message", sql.NVarChar(sql.MAX), {
              nullable: false,
            });
            table.columns.add("is_read", sql.Bit, { nullable: false });
            table.columns.add("created_at", sql.DateTime, { nullable: false });
            ...
            await pool.request().bulk(table);
            console.log(
              `[Scheduler] Sent predictive flood notifications to ${usersResult.recordset.length} users.`,
            );
          }
        } else {
          ...
          await pool
            ...
            .input("description", sql.NVarChar, alertDesc).query(`
        ...
        if (existingAlert) {
          await pool
            .request()
            .input("alert_id", sql.Int, existingAlert.alert_id).query(`
                            UPDATE TrafficAlerts 
                            SET is_active = 0, end_time = GETDATE(), updated_at = GETDATE()
                            WHERE alert_id = @alert_id
                        `);
          console.log(
            `[Scheduler] Cleared weather warning for ${district} because rain stopped.`,
          );
        }
      }
    }
...
async function runCleanupUnverifiedUsersJob() {
  try {
    ...
    console.error("[Scheduler] Cleanup unverified users error:", err.message);
  }
}
...
function startScheduler() {
  ...
  // Job 5: Tự động cào sự kiện từ Danang Fantasticity - mỗi 12 giờ
  cron.schedule("0 */12 * * *", () => {
    console.log("[Scheduler] Running automatic Danang events sync...");
    syncDanangEventsAutomatically();
  });

  // Job 6: Đồng bộ TOÀN BỘ danh mục sự kiện & lễ hội năm (trang tổng hợp) — 1 tuần 1 lần
  // vào lúc 2:00 sáng Chủ Nhật: '0 2 * * 0'. Đây là job đồng bộ chính theo yêu cầu:
  // quét trang danh mục -> AI bóc tách -> so sánh trùng lặp với DB -> pending chờ duyệt.
  cron.schedule("0 2 * * 0", async () => {
    console.log(
      "⏳ [CRON] Đang thực hiện quét tự động danh mục sự kiện định kỳ...",
    );
    await syncYearlyEventCatalog();
  });

  console.log(
    "[Scheduler] Smart Notification & Auto-Sync jobs started successfully!",
  );
}
