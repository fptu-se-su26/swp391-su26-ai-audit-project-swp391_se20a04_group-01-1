...
app.get("/api/user/preferences", authenticateToken, async (req, res) => {
    try {
        ...
        if (result.recordset.length === 0) {
            // Lazy Init: Insert default preferences
            await pool.request()
                .input("user_id", sql.Int, req.user.id)
                .query(`
...
app.put("/api/user/preferences", authenticateToken, async (req, res) => {
    try {
        ...
        // Upsert query using MERGE (very robust in SQL Server)
        await pool.request()
            .input("user_id", sql.Int, req.user.id)
...
// ============ USER FAVORITE PLACES (POIs) ============

// POST /api/pois/:id/favorite - Toggle favorite status
app.post("/api/pois/:id/favorite", authenticateToken, async (req, res) => {
    try {
...
// GET /api/user/favorites/pois - Get favorited POI IDs
app.get("/api/user/favorites/pois", authenticateToken, async (req, res) => {
    try {
        const pool = await poolPromise;
        const result = await pool.request()
            .input("user_id", sql.Int, req.user.id)
...
// GET /api/user/favorites/pois/details - Get favorited POI details
app.get("/api/user/favorites/pois/details", authenticateToken, async (req, res) => {
    try {
        const pool = await poolPromise;
        const result = await pool.request()
            .input("user_id", sql.Int, req.user.id)
...
// Duplicate route PUT /api/user/profile removed. Main route is defined above.
...
// GET /api/user/notifications/unread-count - Đếm số thông báo chưa đọc
...
// PUT /api/user/notifications/:id/read - Đánh dấu 1 thông báo đã đọc
...
// POST /api/user/notifications/test - Gửi test notification cho user hiện tại
app.post('/api/user/notifications/test', authenticateToken, async (req, res) => {
    try {
        const pool = await poolPromise;
        await pool.request()
            .input('user_id', sql.Int, req.user.id)
...
// POST /api/admin/notifications/test-flood - Admin test thủ công gửi flood alert
app.post('/api/admin/notifications/test-flood', authenticateToken, async (req, res) => {
    if (req.user.role !== 'admin') {
...
app
    .listen(PORT, () => {
        ...
        // Khởi động Smart Notification Scheduler
        startScheduler();
    })
    .on("error", (err) => {
